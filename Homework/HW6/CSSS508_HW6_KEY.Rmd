---
title: "Homework 6: 2016 Elections (cont.)"
author: "YOUR NAME"
date: "DATE"
output: 
  html_document:
...
> The first part of this RMarkdown document does not need any modification; it is the key from HW5. The second part is where you should fill in our answers, just like last week! NOTE: BEFORE you start writing code for this week's assignment, run through the provided code from last week.
...
TotalVotes <- executive_races %>% 
  ...
  rename("TotalVotes"= SumOfCount) %>%
  select(-CounterType)
DemVotes <- executive_races %>% 
  select(Precinct,Race,Party,SumOfCount) %>%
  filter(Party %in% c("Dem","DPN")) %>%
  group_by(Precinct,Race) %>% summarize(DemVotes = sum(SumOfCount))
DemPercent <- TotalVotes %>% left_join(DemVotes,by=c("Precinct","Race")) %>%
  mutate(DemPercent = DemVotes/TotalVotes)
DemPercent %>% head(9)
```

## Combining it all

> Once you've calculated democratic voting percentages for *each race* you'll want to put them back together with the precinct turnout rate data using a **join**. Be sure you still have the variable for a precinct being in Seattle or not, as well as the number of registered voters, total number of votes, and votes for democratic candidates.

```{r}
# [YOUR CODE HERE]
Turnout_DemPercent <- Turnout %>%
  left_join(DemPercent %>% select(-Seattle,-TotalVotes),by=c("Precinct","Race"))
```
...
> Using your data from last week, make a scatterplot where the horizontal axis (`x=`) is number of registered voters in the precinct, and the vertical axis (`y=`) is turnout rate. Color (`color=`) the precincts in Seattle one color, and use a different color for other precincts. Do you observe anything?

```{r}
# [YOUR CODE HERE]

ggplot(Turnout,aes(x=RegisteredVotes,y=Turnout,color=Seattle))+geom_point()+
  xlab("Registered Voters")+ylab("Turnout Rate")+
  theme_bw(base_size=15)+
  theme(legend.position = "bottom")
```
> It seems as though Seatle precincts generally have higher turnout rates than Non-Seattle precincts.

### Democratic support

> Now let's visualize the Democratic support rates vs. turnout for the three races and by whether not the precinct was in Seattle. That is, create 2 scatterplots: Each should display Turnout rate on the x axis and Democratic Support rate on the y axis; the color of each point should correspond to the Race. One plot will display data from Precincts in Seattle, while the other should display data from precincts not in Seattle. Use the `facet_wrap` function to display the plots side-by-side. What do you notice?

```{r}
# [YOUR CODE HERE]
ggplot(Turnout_DemPercent,aes(Turnout,DemPercent,color=Race))+geom_point()+
  facet_grid(~Seattle)+
  xlab("Turnout Rate")+ylab("Democratic Voting Percent")+
  ggtitle("Democratic Voting Percentage vs. Turnout, by Race and Location")+
  theme_bw(base_size=15)+
  theme(legend.position = "bottom")
```

> Repeat the previous question, but plotting only precincts with at least 500 registered voters. What do you notice? Do the plots look any different?

```{r}
# [YOUR CODE HERE]
ggplot(Turnout_DemPercent %>% filter(RegisteredVotes>=500),
       aes(Turnout,DemPercent,color=Race))+geom_point()+
  facet_grid(~Seattle)+
  xlab("Turnout Rate")+ylab("Democratic Voting Percent")+
  ggtitle("Democratic Voting Percentage vs. Turnout, by Race and Location","Precincts with >=500 voters")+
  theme_bw(base_size=15)+
  theme(legend.position = "bottom")
```
