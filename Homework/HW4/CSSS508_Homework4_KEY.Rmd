...
> Answer: The "integer" class. This is because vectors convert all data into a single class, and
> because factors have an underlying numeric representation. You can see this for yourself by
> running the code `class( c(vec_int,vec_fac) )`.
...
> Answer: (1) the `logical` class, since it contains boolean values; (2) TRUE's get converted into 0s and FALSE's into 1s, you turn each word into a character string such as "TRUE" and "FALSE", and you get numbers in character form such as "0" and "1"; (3) You get NAs, since R doesn't know how to make this conversion!
...
> Answer: Only `colnames()` returns values (the vector names). This is  because `names()` is meant for
> data.frames, and `rownames()` aren't included in matrices!

2) Consider the code below:
```{r matrices_2, echo=TRUE}
mat_letters <- matrix(letters, ncol=2) 
```
   + This matrix goes from `"a"` to `"m"` in the first column and `"n"` to `"z"` in the second column. 
    What would be an easy way to make the matrix go in alphabetical order left to right, top to bottom?
    
> Answer: Add the argument `byrow=TRUE`, i.e., `mat_letters <- matrix(letters, ncol=2,byrow=TRUE)`
...
> Answer: (1) The vector `math_vec` is added to each column of `math_mat`; (2) the first does proper matrix
> multiplication, while the second does element-by-element multiplication.
...
> Answer: 32 rows and 2 variables
...
> Answer: 37.285126 and -5.344472 (obtained using the code `lm.output$coefficients`).
...
df_4 <- data.frame(y=seq(31,125,length=20),
                   x=runif(20,0,10),
...
# 6) Now convert "df_5" into a data.frame using "data.frame()"
...
> Answer: `colnames()` works the same, but now `names()` and `rownames()` works! This is different
> than on lists and matrices, which only give limited info.


2) Similarly, how do the results of `length()` and `dim()` differ between data frames,
lists, matrices, and vectors?

> Answer: Here, both `length()` and `dim()` work! Length gives the number of columns, and dim works as usual.


3) Using the console, use `==` to test to see if `cbind()` and `data.frame()` in questions 5 and 6 produce the same result. That is, run the code `df_5 == df_6` and describe the answer. What does this tell us about `cbind` and `data.frame`?

> Answer: They produce the same answer! The `==` operator checks element-wise equality, which hold true even
